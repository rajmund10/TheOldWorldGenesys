...
const noSocialStatusLabel = game.i18n.localize('Genesys.Labels.NoSocialStatus');
const specializationLabel = game.i18n.localize('TYPES.Item.specialization');
const noSpecializationLabel = `${specializationLabel}: -`;

const actor = computed(() => {
	rootContext.renderKey;
	return toRaw(rootContext.data.actor);
});

const archetype = computed(() => {
	rootContext.renderKey;
	return actor.value.items.find((i) => i.type === 'archetype') as GenesysItem<ArchetypeDataModel> | undefined;
});

const career = computed(() => {
	rootContext.renderKey;
	return actor.value.items.find((i) => i.type === 'career') as GenesysItem<CareerDataModel> | undefined;
});
...
async function removeSpecialization(specialization?: GenesysItem<SpecializationDataModel>) {
	if (!specialization) {
		return;
	}

	const careerSkills = [...(specialization.systemData.careerSkills ?? [])];
	await toRaw(specialization).delete();
	await toRaw(rootContext.sheet).syncCareerSkillFlags(careerSkills);
}
		...
		<div class="data">
			...
			<div>
				<span v-if="specialization" class="specialization">
					<ContextMenu>
						<template v-slot:menu-items>
							<MenuItem @click="openItemSheet(specialization)">
								<template v-slot:icon><i class="fas fa-edit"></i></template>
								{{ editLabel }}
							</MenuItem>

							<MenuItem @click="removeSpecialization(specialization)" v-if="rootContext.data.editable">
								<template v-slot:icon><i class="fas fa-trash"></i></template>
								{{ deleteLabel }}
							</MenuItem>
						</template>

						<a @click="openItemSheet(specialization)">{{ specialization.name }} <i class="fas fa-arrow-up-right-from-square"></i></a>
					</ContextMenu>
				</span>
