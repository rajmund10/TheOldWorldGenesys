...
async function rollAttack(weapon: GenesysItem<WeaponDataModel>) {
	const initialManualChanges = [
		...getQualityPoolModifications(weapon.systemData.qualities),
		...'D'.repeat(Math.max(0, getQualityCombatEffectValue(weapon.systemData.qualities, CombatEffectKeys.AttackMinimumBrawn) - actor.value.systemData.characteristics.brawn)),
		...'D'.repeat(Math.max(0, getQualityCombatEffectValue(weapon.systemData.qualities, CombatEffectKeys.AttackMinimumAgility) - actor.value.systemData.characteristics.agility)),
	];
