...
const unskilledLabel = game.i18n.localize('Genesys.Labels.Unskilled');
...
async function rollAttack(weapon: GenesysItem) {
	...
	const weaponData = weapon.systemData as WeaponDataModel;
	const initialManualChanges = [
		...getQualityPoolModifications(weaponData.qualities),
		...'D'.repeat(Math.max(0, getQualityCombatEffectValue(weaponData.qualities, CombatEffectKeys.AttackMinimumBrawn) - system.value.characteristics.brawn)),
		...'D'.repeat(Math.max(0, getQualityCombatEffectValue(weaponData.qualities, CombatEffectKeys.AttackMinimumAgility) - system.value.characteristics.agility)),
	];

	await DicePrompt.promptForRoll(toRaw(context.data.actor), attackSkillName, { rollType: RollType.Attack, difficulty: 'DD', initialManualChanges, requireSingleTarget: true, rollData: { weapon }, rollUnskilled });
}
							...
							<ContextMenu v-for="item in equipment.filter((i) => i.type === 'weapon')" :key="item.id" class="weapon">
								<template v-slot:menu-items>
									...
									<MenuItem @click="rollAttack(item)">
										<template v-slot:icon><i class="fas fa-dice"></i></template>
										{{ checkLabel }}
									</MenuItem>
								...
								<a @click="rollAttack(item)" draggable="true" @dragstart="dragStart($event, item)">
									...
									(<span class="weapon-details">
										...
										<span><Localized label="Genesys.Labels.Damage" /> {{ damageForWeapon(item) }}</span>
										<span><Localized label="Genesys.Labels.Critical" /> {{ item.system.critical }}</span>
										<span><Localized label="Genesys.Labels.Range" /> [<Localized :label="`Genesys.Range.${item.system.range.capitalize()}`" />]</span>
