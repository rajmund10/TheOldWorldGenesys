...
const magicAccessoryData = computed(() => system.value as MagicAccessoryDataModel);
...
const displayContainerContents = ref(props.item.type === 'container');
...
async function rollAttack() {
	...
	const actorData = context.data.actor.systemData as CharacterDataModel;
	const initialManualChanges = [
		...getQualityPoolModifications(weaponData.value.baseWeapon.qualities),
		...'D'.repeat(Math.max(0, getQualityCombatEffectValue(weaponData.value.baseWeapon.qualities, CombatEffectKeys.AttackMinimumBrawn) - actorData.characteristics.brawn)),
		...'D'.repeat(Math.max(0, getQualityCombatEffectValue(weaponData.value.baseWeapon.qualities, CombatEffectKeys.AttackMinimumAgility) - actorData.characteristics.agility)),
	];
	...
	>
		...
		<div v-else class="details" @dragenter="dragEnter" @dragleave="dragLeave">
			...
			<div :data-item-type="item.type">
				...
				<!-- Magic Accessory Details -->
				<template v-else-if="item.type === 'magicAccessory'">
					<div>{{ magicAccessoryData.magicSkill || '-' }}</div>
					<div v-if="magicAccessoryData.hasAttackDamageBonus">Atak +{{ magicAccessoryData.attackDamageBonus }}</div>
				</template>
		...
		<button
			v-if="allowEquippedState && canEquipItem(item)"
			type="button"
			class="quick-state equipped"
			data-tooltip="Genesys.Equipment.Dropdown.Equip"
			@click="setItemState(EquipmentState.Equipped)"
		>
			<i class="fas fa-box"></i>
		</button>

		<button
			v-else-if="allowEquippedState && canUnequipItem(item)"
			type="button"
			class="quick-state carried"
			data-tooltip="Genesys.Equipment.Dropdown.Unequip"
			@click="setItemState(EquipmentState.Carried)"
		>
			<i class="fas fa-box"></i>
		</button>
		...
		<ContextMenu class="actions" orientation="left" use-primary-click :disable-menu="!context.data.editable">
			<template v-slot:menu-items>
				...
				<MenuItem v-if="allowEquippedState && item.type === 'magicAccessory' && canEquipItem(item)" @click="setItemState(EquipmentState.Equipped)">
					<template v-slot:icon><i class="fas fa-wand-magic-sparkles"></i></template>
					<Localized label="Genesys.Equipment.Dropdown.Equip" />
				</MenuItem>

				<MenuItem v-if="allowEquippedState && canUnequipItem(item)" @click="setItemState(EquipmentState.Carried)">
					<template v-slot:icon><i class="fas fa-box"></i></template>
				...
				<MenuItem v-if="allowDroppedState && canDropItem(item)" @click="setItemState(EquipmentState.Dropped)">
					<template v-slot:icon><i class="fas fa-box-open"></i></template>
...
.inventory-item {
	display: grid;
	grid-template-columns: [icon] 2rem [name] auto [spacer] 1fr [quantity] 2.5em [weight] 2.5em [damage] 2em [quick-state] 2em [actions] auto;
	...
	&.mini {
		grid-template-columns: [icon] 1.5em [name] auto [spacer] 1fr [quantity] 2.5em [weight] 2.5em [damage] 2em [quick-state] 2em [actions] auto;
		min-height: 2.15em;
		height: auto;
		overflow: visible;
		font-size: 0.8em;
	...
	.quick-state {
		grid-column: quick-state / span 1;
		grid-row: 1 / span 1;
		display: inline-flex;
		align-items: center;
		...
		margin: 0;
		padding: 0;
		border: 1px solid transparentize(colors.$dark-blue, 0.7);
		border-radius: 999px;
		background: transparentize(colors.$blue, 0.86);
		color: colors.$dark-blue;
		line-height: 1;
		text-align: center;
		...
		&:hover {
			background: colors.$gold;
			border-color: transparentize(colors.$gold, 0.25);
			color: white;
			box-shadow: 0 0 0.35em transparentize(colors.$gold, 0.35);
		}

		&.carried {
			background: transparentize(colors.$gold, 0.78);
			border-color: transparentize(colors.$gold, 0.45);
			color: darken(colors.$gold, 20%);
		}

		&.equipped {
			background: transparentize(colors.$blue, 0.86);
			border-color: transparentize(colors.$dark-blue, 0.7);
			color: colors.$dark-blue;
		}
	}
