...
async function showTooltip(event: Event) {
	...
	const plainText = enriched.replace(/<[^>]*>/g, '').trim();

	game.tooltip.activate(event.currentTarget as HTMLElement, { text: plainText, direction: props.direction, cssClass: props.cssClass });
	game.tooltip.tooltip.innerHTML = enriched;
}
