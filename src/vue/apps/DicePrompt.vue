...
onMounted(() => {
	...
	poolModifications.value.manualChanges = [...(context.initialManualChanges ?? [])].sort(sortPoolModifications);
					...
					const totalDefense =
						chosenTarget.actor!.type === 'character'
							...
							: (chosenTarget.actor as GenesysActor<NonVehicleActorDataModel>).systemData.defense[defenseType] +
								getEquippedQualityCombatEffectValue(
									chosenTarget.actor as GenesysActor,
									defenseType === 'melee' ? CombatEffectKeys.DefenseMelee : CombatEffectKeys.DefenseRanged,
									['weapon'],
								);
...
async function rollPool() {
	...
	const baseRollData = {
		...
		formula,
		symbols,
		descriptionOverride: context.rollData?.descriptionOverride,
		chaosManifestation: context.rollData?.chaosManifestation,
	};
