...
export type AttackRollData = {
	weapon: AttackRollWeapon | GenesysItem<WeaponDataModel>;
};
...
export default class DicePrompt extends VueSheet(Application) {
	...
	static async promptForRoll(actor: GenesysActor | undefined, skillName: string, options: DicePromptOptions = {}) {
		if (options.requireSingleTarget) {
			const [target] = game.user.targets;
			if (game.user.targets.size !== 1 || !target?.actor || !['character', 'minion', 'rival', 'nemesis'].includes(target.actor.type)) {
				ui.notifications.warn(game.i18n.localize('Genesys.Notifications.SelectOneTargetForAttack'));
				return;
			}
		}
	...
	constructor(actor?: GenesysActor, skillName?: string, { rollType, difficulty, initialManualChanges, requireSingleTarget, rollUnskilled, rollData }: DicePromptOptions = {}) {
		super();
		...
		this.initialManualChanges = initialManualChanges;
		this.requireSingleTarget = requireSingleTarget;
	...
	override async getVueContext(): Promise<DicePromptContext> {
		return {
			...
			initialManualChanges: this.initialManualChanges,
			requireSingleTarget: this.requireSingleTarget,
